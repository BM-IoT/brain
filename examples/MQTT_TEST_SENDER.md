...
### 1. Start the IoT Brain
```bash
cd src
python3 main.py
```

### 2. Run Test Sender (Auto Mode)
```bash
# Auto-discover broker and run full test sequence
./run_test_sender.sh
...
./run_test_sender.sh --interactive
...
# Specify MQTT broker manually
./run_test_sender.sh --broker 192.168.1.100 1883
...
# Specify broker manually
python3 mqtt_test_sender.py 192.168.1.100 1883
...
When running in interactive mode, you get these options:

1. **Register all devices** - Send network/hello for all test devices
2. **Send sensor data** - Single round of sensor readings
3. **Start continuous stream** - Ongoing data with configurable interval
4. **Send keepalive** - Device heartbeat messages
5. **Remove all devices** - Send network/byebye cleanup
6. **Show device list** - Display all simulated devices
7. **Test threshold breach** - Force warning/critical alerts
...
3. **Run Test Sender**: `./run_test_sender.sh --interactive`
4. **Verify Components**:
